...
cmdline="sudo java --module-path /usr/share/openjfx/lib/ --add-modules=javafx.swing,javafx.graphics,javafx.fxml,javafx.media,javafx.controls,javafx.web,javafx.base -jar iped.jar -o /home/kali/Desktop/IPED-CASO/ -profile fastmode -l /usr/local/bin/iped/palavras-chave.txt"
...
# Verifica se ha Virtual Shadow Copies do Windows montadas (VSS)
while read line ; do
	for vss in $(sudo ls /vss/$line); do
		echo "Achou vss: /vss/$line/$vss"
		sudo mkdir /vss_iped
		# faz um link com a extensao .dd para o disco ser reconhecido como tal pelo IPED
		sudo ln -s /vss/$line/$vss /vss_iped/$line-$vss.dd 
		cmdline+=" -d /vss_iped/$line-$vss.dd"
	done
done <<< "$(sudo ls /vss/)"
